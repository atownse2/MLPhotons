...
#include "ml_photons_saveImage.h"
//
...
class ml_photons_saveImage : public edm::stream::EDProducer<> {
   public:
      explicit ml_photons_saveImage( const edm::ParameterSet& );
      ~ml_photons_saveImage() override;
...
ml_photons_saveImage::ml_photons_saveImage(const edm::ParameterSet& iConfig):
  ...
  cname_(iConfig.getParameter<std::string>("cluster_name"))
{
  ...
  produces<std::vector<std::string>> (cname_+"Img");
...
ml_photons_saveImage::~ml_photons_saveImage()
{
}

// member functions
void
ml_photons_saveImage::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
void
ml_photons_saveImage::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
...
// Now we've got all the clusters
  ...
  //Outputs
  std::unique_ptr<std::vector<std::string>> cluster_Img_( new std::vector<std::string> );
  ...
  for (auto C : Clusters)
  {
    ...
    //float aaa = *C.aimage;
    //cluster_Img_->emplace_back( aaa );
    cluster_Img_->emplace_back( C.simage );
  ...
  iEvent.put(std::move(cluster_Img_), cname_+"Img");
...
void
ml_photons_saveImage::beginStream(edm::StreamID)
{
}

void
ml_photons_saveImage::endStream() {
}

//define this as a plug-in
DEFINE_FWK_MODULE(ml_photons_saveImage);
