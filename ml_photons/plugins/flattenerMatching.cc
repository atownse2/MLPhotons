// -*- C++ -*-
//
// Package:    ML_Photons/flattenerMatching
// Class:      flattenerMatching
// 
/**\class flattenerMatching flattenerMatching.cc ML_Photons/flattenerMatching/plugins/flattenerMatching.cc
...
class flattenerMatching : public edm::one::EDAnalyzer<edm::one::SharedResources, edm::one::WatchRuns> {
   public:
      explicit flattenerMatching( const edm::ParameterSet& );
      ~flattenerMatching() override;
   ...
   private:
      ...
      std::vector<float> jet_matchedEta;
...
flattenerMatching::flattenerMatching(const edm::ParameterSet& iConfig):
...

{
  ...
  tree->Branch("jet_matchedEtaE", &jet_matchedEta );
...
flattenerMatching::~flattenerMatching()
{
}
...
void
flattenerMatching::analyze( const edm::Event & iEvent, const edm::EventSetup & iSetup)
{
  ...
  for (auto jet_iter = patJet->begin(); jet_iter != patJet->end(); ++jet_iter){
    ...
    float jeta = jet_iter->eta();
    float jphi = jet_iter->phi();
    float geta;
    float gphi;

    jet_matchedEta.push_back(-1);

    //Loop through gen particles
    Handle<vector<reco::GenParticle>> genpart;
    iEvent.getByToken(genpartToken, genpart);
    for (auto gp_iter = genpart->begin(); gp_iter != genpart->end(); ++gp_iter){
      if (gp_iter->pdgId() == 221 && gp_iter->numberOfDaughters()==2){
        if( gp_iter->daughter(0)->pdgId() == 22 && gp_iter->daughter(1)->pdgId() == 22 ){
          geta = gp_iter->eta();
          gphi = gp_iter->phi();
          float deta = jeta - geta;
          float dphi = jphi - gphi;
          float deltaR = sqrt( deta*deta + dphi*dphi );
          if(deltaR < 0.4){
            jet_matchedEta.back() = gp_iter->energy();
            break; //Found our eta, break the genparticle loop
          }
        }
      }
    } //genpart loop
...
void
flattenerMatching::beginJob()
{
}

void
flattenerMatching::endJob() {
}

void 
flattenerMatching::beginRun(edm::Run const& iRun, edm::EventSetup const& iSetup)
{
}

void 
flattenerMatching::endRun(edm::Run const&, edm::EventSetup const&)
{
}

void
flattenerMatching::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
DEFINE_FWK_MODULE(flattenerMatching);
